...
with col_left:
    ...
    if my_mode == "讀取檔案模式 (setups.txt)":
        ...
        fixed_members = []
        # 建立 5 個獨立的展開區塊 (Expander)
        for i in range(team_size):
            with st.expander(f"動物 {i+1} 設定", expanded=(i==0)):
                cols = st.columns(5)
                pet_name = cols[0].text_input(f"動物名稱", key=f"my_name_{i}", placeholder="例如: ant")
                pet_atk = cols[1].text_input(f"攻擊力", key=f"my_atk_{i}", placeholder="None")
                pet_hp = cols[2].text_input(f"生命值", key=f"my_hp_{i}", placeholder="None")
                pet_lvl = cols[3].selectbox(f"等級", ["None", "1", "2", "3"], key=f"my_lvl_{i}")
                pet_eq = cols[4].text_input(f"裝備", key=f"my_eq_{i}", placeholder="None")
...
with col_right:
    ...
    if enemy_mode == "讀取檔案模式 (天梯資料)":
        ...
        enemy_fixed = []
        for i in range(team_size):
            with st.expander(f"敵方動物 {i+1} 設定", expanded=False):
                cols = st.columns(5)
                pet_name = cols[0].text_input(f"名稱", key=f"en_name_{i}", placeholder="ant")
                pet_atk = cols[1].text_input(f"攻擊", key=f"en_atk_{i}", placeholder="None")
                pet_hp = cols[2].text_input(f"生命", key=f"en_hp_{i}", placeholder="None")
                pet_lvl = cols[3].selectbox(f"等級", ["None", "1", "2", "3"], key=f"en_lvl_{i}")
                pet_eq = cols[4].text_input(f"裝備", key=f"en_eq_{i}", placeholder="None")
