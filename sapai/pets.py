...
class Pet:
    ...
    def after_attack_trigger(self, trigger):
        ...
        # 🌟 取得 triggeredBy 的種類
        triggered_by = self.ability.get("triggeredBy", {}).get("kind", "")

        # 🌟 允許 FriendAhead (袋鼠/蛇) 與 Self (新版大象)
        if triggered_by not in ["FriendAhead", "Self"]:
            raise Exception(
                "Only triggeredBy FriendAhead or Self implemented for after_attack_trigger"
            )

        # 🌟 判斷觸發條件
        if triggered_by == "FriendAhead":
            ### Check that self is behind the front friend which should have just attacked
        ...
        elif triggered_by == "Self":
            ### 新版大象邏輯：必須是自己剛剛發動攻擊 (自己必須站在最前排，也就是 index 0)
            if self.team.get_idx(self) != 0:
                return activated, targets, possible
        ...
        # 🌟 讀取我們在 data.py 中加入的 repeats 欄位，處理大象 Lv2 與 Lv3 的多次連擊
        repeats = self.ability.get("effect", {}).get("repeats", 1)
        
        for _ in range(repeats):
            # 如果有 repeats，就連續發送多次技能觸發 (讓後方的隊友多次受傷)
